...
#define M_PI 3.1415927f
	...
	typedef struct
	{
		int            n;   // 滤波器阶数的一半（不是调用次数）
		FTR_PRECISION *A;   // 滤波器系数
		FTR_PRECISION *d1;  // 滤波器系数
		FTR_PRECISION *d2;  // 滤波器系数
		FTR_PRECISION *w0;  // 内部状态（历史数据）
		FTR_PRECISION *w1;  // 内部状态（历史数据）
		FTR_PRECISION *w2;  // 内部状态（历史数据）
	} BWLowPass;
